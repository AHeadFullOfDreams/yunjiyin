...
class List:
    ...
    def create(self,path,Litera,num,synonyms):
        self.path = path
        ...
        for i in range(num):
            ...
            for k in range(len(values)):#信息顺序录入表格
                m = len(values) - 1
                for j in range(len(synonyms)):#对于摘要中的信息进行过滤
                    result = values[m].find(str(synonyms[j]))
                    if result == -1:
                        continue
                    else:
...
def MainLoop():
    ...
    listing.create(path,Litera,Id_num,synonyms)
